...
const reviewSQL       = `SELECT * FROM reviews WHERE fid = ? AND uid = ?`
const newReviewSQL    = `INSERT INTO reviews (fid, uid, completed, reviewDate, rating, review) VALUES (?, ?, ?, ?, ?, ?)`
const deleteReviewSQL = `DELETE FROM reviews WHERE fid = ? AND uid = ?`
const editReviewSQL   = `UPDATE reviews SET fid = ?, uid = ?, completed = ?, reviewDate = ?, rating = ?, review = ? WHERE fid = ? and uid = ?`
